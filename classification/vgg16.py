...
import torch
from torchvision.models import resnet18, googlenet, vgg16
...
# model = vgg16(pretrained=False)
print(model)
model.classifier.add_module("add_linear",nn.Linear(1000,200))
# model = vgg16_bn()
...
train_scheduler = torch.optim.lr_scheduler.MultiStepLR(
    optimizer, milestones=[30], gamma=0.2
) 
# 训练和验证模型
...
for epoch in range(num_epochs):
    ...
    for phase in ['train', 'val']:
        ...
        if epoch_acc > best_acc and phase=='val':
            best_acc = epoch_acc
            # torch.save(model, f"results/{args.model}/checkpoints/cifar_{args.mode}_{epoch}.pth")
